    ...
    <>
      ...
      <form className="relative overflow-x-auto p-5 pb-14">
            ...
            {schedules.map((schedule, index) => (
              <tr key={index} className=" even:bg-lithBlue odd:bg-white">
                ...
                <td className="px-6 py-4">
                  <input className="border-y-2 focus:outline-none border-blue bg-transparent text-sm zoom block" type="time" value={schedule.startTime} readOnly />
                </td>
                <td className="px-6 py-4">
        ...
        <div className="flex print:hidden justify-center mt-2">
          <button onClick={handleGeneratedPdf} className="bg-darkBlue flex p-2 flex-row ">
            Generated pdf
            <img className="pl-2" src="/Images/WhiteCalendar.png" alt="calendar" />
          </button>
        </div>
      </form>
    </>
