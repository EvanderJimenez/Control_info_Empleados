import { StarGetAllBosses, StartGetEmployeeByUid, selectGetAllBosses } from '@/root/redux';
  ...
  return (
    ...
    {listBoss?.map((request: EmployeesType, index: number) => (
      <div key={index} className="mb-4 p-2 border border-gray-300 rounded">
        ...
        <button className="bg-darkBlue" onClick={() => handleLoadInformation(request.uid)}>Load Information</button>
      </div>
    ))}
  </div>
  )
